    ...
    <script>
        $( document ).ready( function() 
        {
            ...
            <?php foreach( $pointOfInterests as $key => $row) :?>
                loadMarker(<?php echo $row->pic_map_x; ?>, <?php echo $row->pic_map_y; ?> , <?php echo $row->id; ?> , '<?php echo $row->pic_name; ?>');
            <?php endforeach?>
        });
        
        // Load existing marker
        
        function loadMarker(mapx, mapy , poi = null , poi_name = '')
        {
            ...
            $("#maps").append(            
                $(`<div class="marker map${recentPointer}" data-link='${dataLink}'
                    data-toggle="tooltip" data-placement="top" title="${poi_name}"></div>`).css({       // include a class
                    position: 'absolute',
                    top: (y - 50) + 'px',
                    left: (x - 50) + 'px',
                    width: '50px',
